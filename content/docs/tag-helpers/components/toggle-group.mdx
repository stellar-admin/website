...
<Demo src="/demo/tag-helpers/toggle-group-class-names.html">
  <Tabs items={["Razor", "CSS"]}>
    <Tab value="Razor">
      <include>./_include/toggle-group-class-names.mdx</include>
    </Tab>
    <Tab value="CSS">
      ```css
      .travel-field {
      ...
      .travel-toggle-group-control {
          gap: 0.5rem;
      }

      .travel-toggle-group-item {
          border: 2px solid var(--travel-accent);
          border-radius: 0.75rem;
          color: var(--travel-accent);
      }

      .travel-toggle-group-item:has(input:checked) {
...
Renders a `<div role="group">` element containing the items. When `asp-for` is set, the group model-binds and is wrapped in an implicit [Field](field).
