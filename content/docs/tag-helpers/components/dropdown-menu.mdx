...
<Callout type="tip">
  Control where the menu opens relative to its trigger with the `position` attribute on `<sa-dropdown-menu-content>`. It accepts the same `PositionArea` values as the [Popover](popover) Tag Helper and defaults to opening below the trigger.
</Callout>
...
Renders the menu itself as a `<sel-dropdown-menu>` web component with the [`popover` attribute](https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Global_attributes/popover), anchored to the trigger.
...
Renders a `<div role="menuitem">`, or an [`<a role="menuitem">`](https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/a#attributes) when an `href` or any routing attribute is supplied.
...
The item also accepts the same routing attributes as the ASP.NET Core [Anchor Tag Helper](https://learn.microsoft.com/en-us/aspnet/core/mvc/views/tag-helpers/built-in/anchor-tag-helper) for generating the `href`. Supply either an `href` or these attributes — not both.
