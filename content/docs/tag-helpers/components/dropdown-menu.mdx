...
Groups a trigger and its menu content. It renders no element of its own — it
only generates the shared id (honouring a user-supplied `id`) that connects the
trigger to the menu.
...
Renders a `<button>` styled as a `<sa-button>` that opens the menu via the
native `popovertarget` attribute.
...
Renders the menu itself as a `<sel-dropdown-menu>` web component with the
[`popover`
...
Renders a `<div role="menuitem">`, or an [`<a
...
Renders a `<div role="menuitemcheckbox">` with a leading check indicator.
Toggling it raises a bubbling `checkedchange` event.
...
Renders a `<div role="group">` that enforces single selection among its radio
items and raises a bubbling `valuechange` event when the selection changes.
...
Renders a `<div role="menuitemradio">` with a leading check indicator, checked
when its `value` matches the group's `value`.
...
Groups a sub-trigger and its submenu. Like `<sa-dropdown-menu>`, it renders no
element of its own and only generates the shared id connecting the two.

### `<sa-dropdown-menu-sub-trigger>`

Renders a `<div role="menuitem">` with a trailing chevron that opens its submenu
on hover or with the `→` key.
...
Renders a submenu as a `<sel-dropdown-menu>` popover anchored to its
sub-trigger, accepting the same `position` values as
`<sa-dropdown-menu-content>`.
