...
Renders a `<label>` wrapping a visually hidden native `<input type="checkbox">` that carries the on/off value. When `asp-for` is set, it delegates to the built-in [ASP.NET Core Input Tag Helper](https://learn.microsoft.com/en-us/aspnet/core/mvc/views/working-with-forms#the-input-tag-helper) so the toggle model-binds like any other `bool`.
