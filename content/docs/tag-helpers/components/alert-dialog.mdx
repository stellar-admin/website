...
The `closedby` attribute defaults to `closerequest`, so the dialog is not
light-dismissable.
...
Renders a `<button type="submit">` with `value="cancel"` (unless you supply your
own `value`), so a wrapping `<form method="dialog">` closes the dialog with
that `returnValue`.
...
Renders a `<button type="submit">` with `value="confirm"` (unless you supply
your own `value`), so a wrapping `<form method="dialog">` closes the dialog
