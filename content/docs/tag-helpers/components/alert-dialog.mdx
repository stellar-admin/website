...
<Callout type="tip">
  `alertDialog()` is built on the same primitive as the [`dialog()` helper](js-dialog). Use it whenever you want to drive a confirmation in an async/await style. See the [`alertDialog()` documentation](js-alert-dialog) for the full API.
</Callout>
...
Renders a native [`<dialog>` element](https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/dialog) which is opened and closed via the Invoker Commands API (`command="show-modal"` / `command="close"` on a button whose `commandfor` targets the dialog's `id`). The `closedby` attribute defaults to `closerequest`, so the dialog is not light-dismissable.
