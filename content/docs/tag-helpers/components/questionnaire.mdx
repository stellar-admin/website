...
Each `<sa-questionnaire-item>` renders a `<fieldset>` with its title as the `<legend>`, and each choice is a real radio button or checkbox, so a questionnaire posts and validates like any other form.
...
The actions render submit buttons. Give them a shared `name` and distinct `value` attributes to identify which one was clicked on the server.
