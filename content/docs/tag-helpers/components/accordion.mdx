...
Renders a `<details>` element. Native `<details>` attributes such as `name` and
`open` work as usual, and adding `disabled` renders the item as disabled.

### `<sa-accordion-item-title>`

Renders a `<summary>` element and appends a chevron indicator icon after the
title content.
