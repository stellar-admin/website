...
<Callout type="warn">
  Input OTP only submits a value when you set `asp-for` or `name`. Just like a native input without a `name`, an Input OTP without one (such as most of the examples on this page) renders and is fully interactive but posts nothing. Use `asp-for` to [model bind](#model-binding), or set `name` to post a value without binding.
</Callout>
...
<sa-input-otp name="verificationCode" label="Verification code" max-length="6"
              class-names="@partClasses" />
```

Define the custom class in your stylesheet.
...
| `Input` | The transparent native input over the slots that receives the code. |
| `Group` | Every group of character slots. |
| `Slot` | Every character slot. |
| `Separator` | Every separator between groups. |
| `Caret` | The active slot's caret container. |
| `CaretLine` | The line inside the active caret. |
...
Group, Slot, and Separator apply to both automatically generated parts and child Tag Helpers you compose inside the OTP input. Classes on individual child elements are added alongside these shared classes. Caret and CaretLine apply when the active caret is rendered.

## API Reference

### `<sa-input-otp>`

Renders a `<sel-input-otp>` web component containing the slot cells and a single transparent `<input>` element that holds the whole code and posts it as one form value. When `asp-for` is set, it delegates to the built-in [ASP.NET Core Input Tag Helper](https://learn.microsoft.com/en-us/aspnet/core/mvc/views/working-with-forms#the-input-tag-helper) to resolve the input's name, value, and validation attributes.
