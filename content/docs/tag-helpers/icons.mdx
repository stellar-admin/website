...
StellarAdmin Tag Helpers ships the [Lucide](https://lucide.dev/) icon set. It is registered by `AddStellarAdmin()`, so every Lucide icon is available from the [`<sa-icon>`](/docs/tag-helpers/components/icon) Tag Helper by name, without further setup:
...
Supply glyphs compatible with component styling: loading icons rotate continuously, carousel navigation adapts to orientation and text direction, and `ScrollToEnd` rotates for scrolling to the start. Inline SVGs such as the accordion indicator, dashboard view icons, and application-selected icon names are outside this semantic mapping.
