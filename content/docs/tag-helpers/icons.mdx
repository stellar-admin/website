...
StellarAdmin ships the [Lucide](https://lucide.dev/) icon set. It is registered by `AddUI()`, so every Lucide icon is available from the [`<sa-icon>`](/docs/tag-helpers/components/icon) tag helper by name, without further setup:
