...
Use this for layout and spacing utilities, or for your own CSS classes. If your project runs its own Tailwind CSS build, utilities such as `w-full` or `bg-primary` work as usual (see [Theming](/docs/tag-helpers/theming#using-the-design-tokens-in-your-own-app) for making them resolve to the StellarAdmin design tokens). Without a Tailwind build, only the utility classes that ship in the theme stylesheet are available, so add your own classes and style them in your stylesheet.
