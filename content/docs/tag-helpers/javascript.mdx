...
| `<sa-questionnaire>` | `<sel-questionnaire>` | The shortcut keys assigned by `<sa-questionnaire-choices>`, arrow keys that move between a question's answers and wrap at either end, and a free-text answer replacing the choice a single-answer question holds. |
