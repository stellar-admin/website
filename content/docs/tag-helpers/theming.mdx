...
Floating menus (currently the [Dropdown Menu](/docs/tag-helpers/components/dropdown-menu) content and sub-menus) have three application-wide appearance settings, configured once in `Program.cs` by chaining `ConfigureMenu` off `AddTagHelpers()`:
