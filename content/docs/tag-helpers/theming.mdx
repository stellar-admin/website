...
StellarAdmin ships nine themes: Ledger, Luma, Lyra, Maia, Mira, Nova, Rhea, Sera, and Vega. Use the picker above any component example to compare them. The eight shadcn-derived themes share a base palette and differ in shape, spacing, and component styling. Ledger is an independently designed theme with its own palette, typography, and raised controls.
...
<Callout type="idea">
    Since the StellarAdmin Tag Helpers are based on shadcn/ui, you can also use their [Create page](https://ui.shadcn.com/create) to browse the different themes. The *Style* selector on the left-side of the _Create_ page corresponds with the eight upstream-derived StellarAdmin themes. Ledger is available through the demo picker on this site.
</Callout>


## Ledger

Ledger pairs warm paper surfaces in light mode with blue-tinted charcoal in dark mode, an indigo primary colour, and 6px corners. Primary, secondary, outline, and destructive buttons have visible borders and a raised edge; pressed buttons sink by 1px, and keyboard focus retains their elevation. Ghost and link actions stay flat.

Link the Ledger bundle and load Lexend for UI text and JetBrains Mono for identifiers and shortcuts. Fonts are supplied by your application; the bundle does not download them. You can self-host them or use a font provider:
...
<link rel="stylesheet" href="/_content/StellarAdmin.TagHelpers/stellar-admin.ledger.css" />
```

Use the same tag helpers and dark-mode class as other themes. Ledger also styles the shared components used by StellarAdmin Pro, including the card and table surfaces used by the data grid. Standard semantic colour variables remain customisable. Variables beginning with `--sa-ledger-` implement Ledger-specific treatments and are not a shared cross-theme token contract.

Ledger's primary and destructive buttons use dedicated hover and border colours. If you replace those brand colours, also adjust `--sa-ledger-primary-hover`, `--sa-ledger-primary-border`, `--sa-ledger-destructive-hover`, `--sa-ledger-destructive-border`, and `--sa-ledger-destructive-foreground` in each colour mode so the raised treatment remains coherent.
